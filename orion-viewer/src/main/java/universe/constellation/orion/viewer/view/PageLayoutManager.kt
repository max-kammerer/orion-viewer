...
class PageLayoutManager(val controller: Controller, val scene: OrionDrawScene) {
    ...
    fun performTouchZoom(zoom: Float, startFocus: PointF, endFocus: PointF) {
        ...
        //TODO delete pages
        activePages.forEach {
            it.reinit(operation = Operation.PINCH_ZOOM)
        }
    }
...
//        logNativeMemory("scroll: ${activePages.size} active pages")
    ...
    private fun scrollPageTo(page: PageView, x: Int, y: Int, isTapNavigation: Boolean) {
        val oldPosition = page.layoutData.position
    ...
    fun renderPageAt(
        ...
        y: Int,
        isTapNavigation: Boolean = false,
        doScroll: (PageView, PageInfo) -> Unit = { page, _ -> scrollPageTo(page, x, y, isTapNavigation) }
    ): PageView? {
